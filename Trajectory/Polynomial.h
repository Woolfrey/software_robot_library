...
class Polynomial
{
	...
	private:
		...
		float *coeff;                                                                              // Array of coefficient [mxn]
					   ...
					   R0(startPoint)
{
	if(times_are_sound(startTime, endTime))
	{
		...
		Eigen::AngleAxisf dR = initialRotation.inverse()*finalRotation;                            // R0*dR = Rf ---> dR = R0^-1*Rf
		...
		this->isValid = compute_coefficients(startPoint, endPoint, startVelocity, endVelocity);
	}	
}
									  ...
									  const std::vector<float> &endVelocity)
{
	...
	this->coeff = new float [m][n];                                                                // Resize array

	if(this->coeff[0][0] == nullptr)
	{
						   ...
						   const float &time)
{
	...
	else if(pos != this->m or vel != this->m or acc != this->m)
	{
		std::cerr << "[ERROR] [POLYNOMIAL] get_state(): "
