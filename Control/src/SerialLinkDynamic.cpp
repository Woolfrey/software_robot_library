...
SerialLinkDynamic::resolve_endpoint_motion(const Eigen::Vector<double,6> &endpointMotion)
{
    ...
    if (not is_singular())
    {
        if (numJoints <= 6)
        {
            ...
            Vector<double,6> dynamicForce = (_jacobianMatrix * inertiaMatrix.ldlt().solve(_jacobianMatrix.transpose())).ldlt().solve(endpointAcceleration);
            
            controlAcceleration = QPSolver<double>::solve(
                inertiaMatrix,                                                                      // H
               -_jacobianMatrix.transpose() * dynamicForce,                                         // f
                _constraintMatrix,                                                                  // B
                _constraintVector,                                                                  // z
