...
class SerialDynamicControl : public SerialLinkBase <DataType>
{
    public:
         ...
         * @param desiredAcc The desired joint accelerations (nx1).
        ...
        else                                                                                      // Redundant robot
        {
            ...
            if(not this->_redundantTaskSet)
            {
                this->_redundantTask = (this->_controlFrequency/50)                             // NOTE: NEED TO EXPERIMENT TO DETERMINE SCALAR
                                       *this->manipulability_gradient()
                                       + this->_model->joint_damping_vector();                        // Autonomously reconfigure the robot away from singularities
            }
                                                          ...
                                                          const Eigen::Vector<DataType,6> &desiredAcceleration)
{
    ...
    return resolve_endpoint_motion(desiredAcceleration
                                    + this->_cartesianStiffness*actualPose.error(desiredPose)
                                    + this->_cartesianDamping*(desiredVelocity-this->_jacobianMatrix*this->_model->joint_velocities()));       // Feedforward + feedback control
}
