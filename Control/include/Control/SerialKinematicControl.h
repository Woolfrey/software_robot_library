...
#include "SerialLinkBase.h"                                                                         // The " " tells the compiler to look in the same folder
...
SerialKinematicControl<DataType>::resolve_endpoint_motion(const Eigen::Vector<DataType,6> &endpointMotion)
{
    ...
    else // SINGULAR
    {
        ...
        controlVelocity =
        QPSolver<DataType>::solve(Eigen::Matrix<DataType,Eigen::Dynamic,Eigen::Dynamic>::Zero(numJoints,numJoints),
                                  Eigen::Vector<DataType,Eigen::Dynamic>::Zero(numJoints),
